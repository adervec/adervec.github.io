...
self.addEventListener("activate",e=>{e.waitUntil(caches.keys().then(ks=>Promise.all(ks.filter(k=>k.startsWith("portal-")&&k!==C).map(k=>caches.delete(k)))));});
